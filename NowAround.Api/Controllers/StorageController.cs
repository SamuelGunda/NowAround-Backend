...
public class StorageController(IStorageService storageService) : ControllerBase
{
    ...
    [Authorize]
    public async Task<IActionResult> UploadImageAsync(IFormFile image, [FromRoute] string auth0Id, [FromRoute] string type, [FromRoute] string id)
    {
        if (AuthorizationHelper.HasAdminRightsOrMatchingAuth0Id(User, auth0Id))
        {
            ...
            await storageService.UploadImageAsync(image, role, auth0Id, type, id);
